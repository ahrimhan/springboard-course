...
# $$
# # Submitted by Ahrim Han (2019/3/18)
#%% [markdown]
...
sns.lmplot(x="Weight", y="Height", hue="Gender", data=dflog, fit_reg=False, height=8,
...
scores = []
for Cp in Cs:
    average = cv_score(LogisticRegression(C = Cp, solver='lbfgs'), Xlr, ylr)
    print("C={}, Score={}".format(Cp, average))
    scores.append(average)

index, value = max(enumerate(scores), key=operator.itemgetter(1))
best_C = Cs[index]
print("\nMax Accuracy: ", value)
print("Optimum C: ", best_C)
...
#%% [markdown]
# The accurancy is turned out be exactly same as what was found using randon train-test split data. 
# However, the cross-validation is necessary to avoid overfitting issue. 
# By cross-validation, we make train and test the model on different data sets.
# If only train data from the train-test split is used (i.e., one fold), then the model may not perform very well on the new data. 
#%% [markdown]
...
clf = sklearn.model_selection.GridSearchCV(model, param_grid={'C': Cs}, cv=5)
clf.fit(Xlr, ylr)
print("C={}, Score={}".format(clf.best_estimator_.C, clf.best_score_))
